...
emulator math utilities.

Source: v2ecore/emulator_utils.py (authors: Yuhuang Hu, Tobi Delbruck).
Copied verbatim except for dropping the `if __name__ == "__main__"` CUDA-only
demo block at the end -- every function below is untouched math (lin-log
intensity mapping, IIR lowpass filtering, leak current, shot noise, event
quantization, photoreceptor noise voltage). See v2e_emulator.py's module
docstring for why this is vendored rather than depended on directly.
"""
import logging
import math
